...
import cardHouse from '@/static/images/cardHouse.png';
    ...
    return dataSource.data.map((item) => {
      return (<Cell colSpan={4} className={styles.cardWarp}>
        <Card free>
          <Card.Actions>
            <p style={{ float: 'right' }}>
              <Balloon trigger={leftTop} align="br" alignEdge triggerType="click">
                <div>
                  <p style={{textAlign:'center'}}>告警信息：</p><p>{item.warning}</p>
                </div>
              </Balloon>
            </p>
            {/*<p style={{ float: 'right' }}>*/}
            {/*  <Img src={cardHouse} height={70} width={160}/>*/}
            {/*</p>*/}
          ...
          <Card.Actions>
            <p className={styles.cardContent}>
              ...
              <span style={{
                float: 'right',
                color: '#666666',
              }}>工作状态：<span className={styles.deviceStatus}>{item.deviceStatus}</span></span>
            </p>
          </Card.Actions>
        </Card>
      </Cell>);
